...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(httptest); setwd(file.path(.libPaths()[1], "useRsnap", "tests")); system.time(test_check("useRsnap", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-export _R_CHECK_CRAN_INCOMING_REMOTE_=FALSE && R CMD check --as-cran useRsnap_$(VERSION).tar.gz
	rm -rf useRsnap.Rcheck/
